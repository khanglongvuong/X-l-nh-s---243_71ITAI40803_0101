...
print("--- Bắt đầu xử lý Lọc ảnh với Butterworth Highpass Filter ---")
...
for filename in os.listdir(image_directory):
    ...
    if os.path.isfile(file_path) and any(filename.lower().endswith(ext) for ext in image_extensions):
        ...
        try:
            ...
            # Tạo bộ lọc Butterworth Highpass
            ...
            for i in range(M):
                for j in range(N):
                    ...
                    # Áp dụng công thức Butterworth Highpass Filter
                    if r == 0: # Xử lý trường hợp chia cho 0 tại tâm
                        H[i, j] = 0.0 # Tần số DC bị loại bỏ trong Highpass
                    else:
                        H[i, j] = 1 / (1 + (d_0 / r)**(2 * n_order))
            ...
            axes[1].set_title(f"BHPF (D0={d_0}, n={n_order}) - {filename}")
            axes[1].axis('off')

            plt.suptitle("Lọc ảnh trong miền tần suất (Butterworth Highpass Filter)")
            plt.show()

        except Exception as e:
            print(f"Lỗi khi xử lý {filename}: {e}")

print("--- Hoàn thành xử lý Lọc ảnh với Butterworth Highpass Filter ---")